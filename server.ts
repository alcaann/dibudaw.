...
mongoose.connect( /* introducir url de base de datos mongo db! (string) */, {
...
function run(): void {
  ...
  const socketServer = sockets.socketApp();
  socketServer.listen(socketPort, () => {
    console.log(`socketServer escuchando en el puerto ${socketPort}`);
  })

 const socialPort = 3001;
 const socialServer = social.socialApp();
 socialServer.listen(socialPort, () => {
   console.log(`socialServer escuchando en el puerto ${socialPort}`);
 })
